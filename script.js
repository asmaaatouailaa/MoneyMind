...
const tourSteps = [
  {
    ...
    content: 'This is your financial dashboard. From here you can access all our tools to help manage your finances.',
    position: 'bottom'
  },
  {
    target: '#quiz',
    title: 'Financial Knowledge Quiz',
    content: 'Test your financial knowledge with our interactive quiz and learn important concepts.',
    position: 'top',
    scroll: true
  },
  {
    target: '#debt-calculator',
    title: 'Debt Calculator',
    content: 'Use our debt calculator to plan your repayment strategy and become debt-free faster.',
    position: 'top',
    scroll: true
  },
  {
    ...
    content: 'Create personalized budgets with our planner to track your income and expenses.',
    position: 'top',
    scroll: true
  },
  {
    target: '.logo',
    title: 'Enjoy MoneyMind!',
    content: 'You\'re all set! Explore all our financial tools and improve your financial health.',
    position: 'bottom',
    scroll: true
  }
];
...
const skipButton = document.getElementById('skip-tour');
...
function showStep() {
  ...
  setTimeout(() => {
    ...
    tourText.textContent = step.content;
    
    // Position the tour box relative to the target
    if (targetElement) {
      ...
      switch (step.position) {
        case 'top':
          topPosition = rect.top - tourContainer.offsetHeight - 20;
          leftPosition = rect.left + (rect.width / 2) - (tourContainer.offsetWidth / 2);
          break;
        case 'bottom':
      ...
      // Ensure the tour box stays within viewport
      topPosition = Math.max(20, Math.min(topPosition, windowHeight - tourContainer.offsetHeight - 20));
      leftPosition = Math.max(20, Math.min(leftPosition, windowWidth - tourContainer.offsetWidth - 20));
      
      tourContainer.style.top = `${topPosition}px`;
      tourContainer.style.left = `${leftPosition}px`;
      tourContainer.style.transform = 'none';
    } else {
      // Center if no target
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!localStorage.getItem('tourCompleted')) {
    setTimeout(startTour, 1000); // Start tour after 1 second
  }
});
